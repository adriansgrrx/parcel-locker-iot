...
try {
    ...
    // === Get parcel counts - PERMANENT STORAGE ===
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['get_parcel_counts'])) {
        try {
            $stmt = $pdo->query("SELECT parcel_count_c1, parcel_count_c2 FROM control_flags WHERE id = 1");
            $counts = $stmt->fetch();
            
            if ($counts) {
                echo json_encode([
                    'success' => true,
                    'c1_count' => (int)$counts['parcel_count_c1'],
                    'c2_count' => (int)$counts['parcel_count_c2'],
                    'total_count' => (int)$counts['parcel_count_c1'] + (int)$counts['parcel_count_c2']
                ]);
            } else {
                echo json_encode([
                    'success' => true,
    ...
    // === Weekly Chart Data for Dashboard ===
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['weekly_data'])) {
        try {
            // Group events by day for the last 7 days
            $stmt = $pdo->query("
                SELECT DATE(created_at) as day, COUNT(*) as count
                FROM events_log
                WHERE event_type = 'parcel_detected' AND created_at >= CURDATE() - INTERVAL 6 DAY
                GROUP BY day
                ORDER BY day ASC
            ");
            ...
            for ($i = 6; $i >= 0; $i--) {
                ...
                $label = date('D', strtotime($date)); // e.g., Mon, Tue
                $map[$label] = 0;
            }

            foreach ($rows as $row) {
                $label = date('D', strtotime($row['day']));
                $map[$label] = (int)$row['count'];
            }
            ...
            $peakDay = $labels[array_search(max($values), $values)];
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // === Regular data update with ENHANCED PARCEL COUNTING LOGIC ===
        ...
        $currentStatusStmt = $pdo->prepare("SELECT status FROM compartments WHERE compartment_id = ?");
        $currentStatusStmt->execute([$compartment_id]);
        $currentRow = $currentStatusStmt->fetch();
        $currentStatus = $currentRow ? $currentRow['status'] : 'Empty';

        // Update compartment status
        $stmt = $pdo->prepare("UPDATE compartments SET distance_cm=?, is_parcel_detected=?, is_security_mode=?, status=? WHERE compartment_id=?");
        $stmt->execute([$distance_cm, $is_parcel_detected, $is_security_mode, $status, $compartment_id]);

        // === ENHANCED PARCEL COUNTING LOGIC - PERMANENT STORAGE ===
        // Check if status changed from Empty to Occupied (new parcel delivered)
        if ($currentStatus === 'Empty' && $status === 'Occupied') {
            // Determine which compartment counter to increment
            $counterColumn = '';
            if ($compartment_id === 'C1') {
                $counterColumn = 'parcel_count_c1';
            } elseif ($compartment_id === 'C2') {
                $counterColumn = 'parcel_count_c2';
            }

            if ($counterColumn) {
                // Increment the parcel counter permanently
                $counterStmt = $pdo->prepare("UPDATE control_flags SET $counterColumn = $counterColumn + 1 WHERE id = 1");
                $counterStmt->execute();

                // Get the new count for logging
                $newCountStmt = $pdo->query("SELECT $counterColumn as new_count FROM control_flags WHERE id = 1");
                $newCountRow = $newCountStmt->fetch();
                $newCount = $newCountRow['new_count'];

                // Log the parcel delivery event with the new count
                $event_type = 'parcel_detected';
                $logStmt = $pdo->prepare("INSERT INTO events_log (event_type, compartment_id, status, distance_cm) VALUES (?, ?, ?, ?)");
                $logStmt->execute([$event_type, $compartment_id, $status, $distance_cm]);

                // Optional: Log a special event for parcel counting
                $countLogStmt = $pdo->prepare("INSERT INTO events_log (event_type, compartment_id, status, distance_cm) VALUES (?, ?, ?, ?)");
                $countLogStmt->execute(['parcel_detected', $compartment_id, "Count: $newCount", $distance_cm]);
            }
        } else {
            // Log regular events (non-counting events)
            $stmt = $pdo->prepare("INSERT INTO events_log (event_type, compartment_id, status, distance_cm) VALUES (?, ?, ?, ?)");
    ...
    // === Fetch frontend dashboard data ===
        ...
        // Get parcel counts - these are now permanent totals
        $countsStmt = $pdo->query("SELECT parcel_count_c1, parcel_count_c2 FROM control_flags WHERE id = 1");
        ...
        echo json_encode([
            ...
            "events" => $events,
            "parcel_counts" => [
                "c1" => (int)($counts['parcel_count_c1'] ?? 0),
                "c2" => (int)($counts['parcel_count_c2'] ?? 0),
                "total" => (int)($counts['parcel_count_c1'] ?? 0) + (int)($counts['parcel_count_c2'] ?? 0)
            ]
        ]);
        exit;
    }
            ...
            <div class="bg-white/95 rounded-2xl p-6 sm:p-8 shadow-xl mb-8 sm:mb-12">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="refreshChart()" class="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors duration-200 text-sm font-medium">
                            🔄 Refresh
                        </button>
                    </div>
                </div>
                ...
                <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                    ...
                    <div class="bg-green-50 rounded-lg p-3">
                        ...
                        <div class="text-lg font-bold text-green-600" id="dailyAverage">0</div>
                    </div>
